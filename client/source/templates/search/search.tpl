template () {
  ...
  URawPrefixInput = state(Unit String, ""),
  UEnteredInput = state(Unit String, ""),
  FreshInputValue = state(Unit String, ""),
  USelectedIndex = state(Unit Number),
  ShowPrefixMatches = state(Unit Null),

  RawPrefixInput = fetch URawPrefixInput,
  EnteredInput = fetch UEnteredInput,
  SelectedIndex = fetch USelectedIndex,

  // ==============================================================
  // Build Indices
  // ==============================================================

  ParseWords = function (s::String)::Set (Tuple2 String Number) {
    var input = s.replace(/[^A-Z^a-z^ ]*/g, "");
    input = input.toLowerCase();
    var words = input.split(" ");
    words = map(words, function(value) {
      return makeTuple(value, 10);
    });
  ...
  MakeSearchResult = function (word::String, doc::String)::SearchResult {
    return doc;
  },

  ForwardIndex = buildMap ParseWords Docs,
  SimpleForwardIndex = mapMapValue (mapSet fst) ForwardIndex,

  SimpleInvertedIndex = invert SimpleForwardIndex,

  lookupSR = ()

  InvertedIndex = mapMapKeyValue (word -> docSet -> mapSet (doc -> lookupSR doc word) docSet) SimpleInvertedIndex,

  Terms = keys InvertedIndex,

  InvertedSearchResults = mapMapKeyValue (word -> doc -> mapSet (MakeSearchResult word) doc) InvertedIndex,
  ...
  PrefixInput = ProcessWord RawPrefixInput,
  PrefixMatches = rangeByKey (unfetch PrefixInput) (unfetch (nextWord PrefixInput)) Terms::Set String,

  SearchInput = ProcessWord EnteredInput,
  SearchResults = fetch (getKey SearchInput InvertedSearchResults),
  ...
  DrawSearchResults = template (Results::Set SearchResult, Input::String) {
  ...
  DrawPrefixMatches = template (Matches::Set String) {
    <f:each Matches as Match>
      MyPosition = fetch (getPosition Match Matches),
      if USelectedIndex as _ {
        Selected = equal MyPosition SelectedIndex::Unit Bool,
        if boolToUnit Selected as _ {
      ...
      <input type="text" value="{FreshInputValue}">
        ...
        <f:on keyup>
          extract boolToUnit (or (or (equal event.keyCode 8) (equal event.keyCode 46)) (and (greaterThan event.keyCode 64) (lessThan event.keyCode 123))) as _ {
            set URawPrefixInput event.value,
            set ShowPrefixMatches null,
            unset USelectedIndex
          },
        </f:on>
        <f:on keydown>
          extract boolToUnit (equal event.keyCode 40) as _ {
            if USelectedIndex as currentIndex {
              set USelectedIndex (plus currentIndex 1)
            } else {
              set USelectedIndex 0
            }
          },
          extract boolToUnit (equal event.keyCode 38) as _ {
            set USelectedIndex (subtract SelectedIndex 1)
          },
          extract boolToUnit (equal event.keyCode 13) as _ {
            if USelectedIndex as SelectedIndex {
              SelectedTerm = fetch (getByPosition SelectedIndex PrefixMatches),
              extract (unfetch SelectedTerm) as SelectedTerm {
                set UEnteredInput SelectedTerm,
                set FreshInputValue SelectedTerm
              }
            } else {
              set UEnteredInput event.value
            },
            unset ShowPrefixMatches,
            unset USelectedIndex
          }
        </f:on>
      </input>
