template (videoTimeline::VideoTimeline) {
	...
	drawState = template(className::String, hover::Bool, widthS::Unit Number, heightS::Unit Number, offsetS::Unit Number, durationS::Unit Number) {
		...
		<f:wrapper>
			<div style-position="absolute" style-left="{timeToPixels 0}" style-width="{durationToWidth loadedTime}" style-height="{height}" style-background="rgba(128,128,128,0.5)"/>
			<f:each scrubberTicks as tickTime>
				x = timeToPixels (product videoDuration tickTime),
				<div style-position="absolute" style-left="{x}" style-height="{height}" style-width="1" style-background="#444"/>
			</f:each>
			...
			<f:each timepoints as timepoint>
				...
				<f:wrapper>
					<div style-position="absolute" style-left="{x}" style-height="{height}" style-width="1" style-background="rgba(255,255,255,0.5)">
					...
					<f:each reactiveEqual (fetch hoveredInfon) infon as _>
						<div style-position="absolute" style-left="{x}" style-height="{height}" style-width="1" style-background="orange"/>
					</f:each>
				</f:wrapper>
			</f:each>
			...
			<f:each timeintervals as timeinterval>
				...
				<f:wrapper>
					<div style-position="absolute" style-left="{start}" style-width="{duration}" style-height="{height}" style-background="rgba(255,255,255,0.5)">
						<f:each boolToUnit hover as _><f:call>hoveredInfonEvents infon</f:call></f:each>
					</div>
					<f:each reactiveEqual (fetch hoveredInfon) infon as _>
						<div style-position="absolute" style-left="{start}" style-width="{duration}" style-height="{height}" style-background="orange"/>
					</f:each>
				</f:wrapper>
			</f:each>
		</f:wrapper>
	...
	<f:wrapper>
		...
		<div style-position="absolute" style-width="{scrubberWidth}" style-height="{scrubberHeight}">
			...
			<f:wrapper>
				...
				<div style-position="absolute" style-width="100%" style-height="{zoomHeight}" style-background="#CCC"> // background
					<f:call>drawState "zoom" true (unfetch scrubberWidth) (unfetch zoomHeight) zoomStartS zoomDurationS</f:call>

					<div style-position="absolute" style-left="{timeToZoomPixels selectStart}" style-height="{zoomHeight}" style-width="1" style-background="#CCF"/>
					<div style-position="absolute" style-left="{timeToZoomPixels selectStart}" style-width="{durationToZoomWidth selectDuration}" style-height="{zoomHeight}" style-background="rgba(192,192,255,0.5)"/>

					<div style-position="absolute" style-left="{timeToZoomPixels previewTime}" style-height="{zoomHeight}" style-width="1" style-background="#FFF"/>
				</div>
			</f:wrapper>
			...
			<f:wrapper>
				...
				<div style-position="absolute" style-top="{zoomHeight}" style-width="100%" style-height="{scrollHeight}" style-background="#AAF"> // background
					<div style-position="absolute" style-left="{timeToScrollPixels zoomStart}" style-width="{durationToScrollWidth zoomDuration}" style-height="{scrollHeight}" style-background="#AAA"/>
